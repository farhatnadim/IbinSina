...
from typing import List, Optional, Dict, Union, Tuple, TYPE_CHECKING
...
class MILDataset:
    ...
    def create_cv_splits(
        ...
        test_frac: float = 0.2,
        seed: int = 42,
    ) -> Tuple['MILDataset', List[Tuple['MILDataset', 'MILDataset']]]:
        """
        Create cross-validation splits with a fixed held-out test set.

        1. Stratified split to create fixed test set
        2. StratifiedKFold on remaining data for n folds

        WARNING: This splits by slide_id. If you have multiple slides per patient,
        use .concat_by('case_id').create_cv_splits() to avoid data leakage.

        Args:
            num_folds: Number of CV folds (default: 5)
            test_frac: Fraction of data to hold out for testing (default: 0.2)
            seed: Random seed for reproducibility (default: 42)

        Returns:
            test_dataset: Fixed held-out test set
            folds: List of (train_dataset, val_dataset) tuples for each fold
        """
        from sklearn.model_selection import train_test_split, StratifiedKFold

        slide_ids = self.df.slide_id.values
        ...
        test_dataset = self.get_subset(test_ids.tolist())

        # Step 2: Generate n folds from trainval set
        skf = StratifiedKFold(n_splits=num_folds, shuffle=True, random_state=seed)
        folds = []

        for train_idx, val_idx in skf.split(trainval_ids, trainval_labels):
            fold_train_ids = trainval_ids[train_idx].tolist()
            fold_val_ids = trainval_ids[val_idx].tolist()

            train_dataset = self.get_subset(fold_train_ids)
            val_dataset = self.get_subset(fold_val_ids)
            folds.append((train_dataset, val_dataset))

        print(f"\nCV splits created (seed={seed}):")
        print(f"  Test set: {len(test_dataset)} slides ({test_frac*100:.0f}%)")
        print(f"  TrainVal: {len(trainval_ids)} slides")
        print(f"  Folds: {num_folds}")
        for i, (train_ds, val_ds) in enumerate(folds):
            print(f"    Fold {i}: train={len(train_ds)}, val={len(val_ds)}")

        return test_dataset, folds
...
class GroupedMILDataset:
    ...
    def create_cv_splits(
        ...
        test_frac: float = 0.2,
        seed: int = 42,
    ) -> Tuple['GroupedMILDataset', List[Tuple['GroupedMILDataset', 'GroupedMILDataset']]]:
        """
        Create cross-validation splits with a fixed held-out test set.
        ...
        group_ids = self.group_df[self.group_column].values
        labels = self.group_df.label.values
        ...
        print(f"\nCV splits created (seed={seed}):")
        print(f"  Test set: {len(test_dataset)} groups ({test_frac*100:.0f}%)")
        print(f"  TrainVal: {len(trainval_ids)} groups")
        print(f"  Folds: {num_folds}")
        for i, (train_ds, val_ds) in enumerate(folds):
            print(f"    Fold {i}: train={len(train_ds)}, val={len(val_ds)}")
...
class HierarchicalMILDataset:
    ...
    def create_cv_splits(
        ...
        test_frac: float = 0.2,
        seed: int = 42,
    ) -> Tuple['HierarchicalMILDataset', List[Tuple['HierarchicalMILDataset', 'HierarchicalMILDataset']]]:
        """
        Create cross-validation splits with a fixed held-out test set.

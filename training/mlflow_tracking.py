"""MLflow tracking wrapper with offline fallback."""
...
class MLflowConfig:
    ...
    enabled: bool = True  # Set False to disable tracking entirely


class MLflowTracker:
    """Wrapper for MLflow with graceful degradation to offline mode."""
    ...
    @contextmanager
    def start_run(self, run_name: Optional[str] = None, nested: bool = False, tags: Optional[Dict[str, str]] = None):
        """
...
def create_tracker(config: 'ExperimentConfig') -> Optional[MLflowTracker]:
    """
    Create an MLflow tracker from experiment config.

    Args:
        config: ExperimentConfig with mlflow settings

    Returns:
        MLflowTracker instance or None if tracking is disabled
    """
    if hasattr(config, 'mlflow') and config.mlflow.enabled:

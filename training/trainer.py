...
class MILTrainer:
    ...
    # Metric name aliases (user-facing -> calculate_metrics key)
    METRIC_ALIASES = {"kappa": "quadratic_kappa"}

    def _get_early_stopping_metric(self, val_metrics: Dict[str, float]) -> float:
        """Get the metric value to use for early stopping."""
        metric_key = self.METRIC_ALIASES.get(
            self._early_stopping_metric_name,
            self._early_stopping_metric_name,
        )
